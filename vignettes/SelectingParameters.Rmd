...
```{r, echo = F}
tab <- rbind(
c("`analysis_main`", "`betas` and `sigma_main`, `tau_main` (in beta regression) or `shape_main` (in parametric survival models)
  (also `D_main` in multi-level models and `basehaz` in proportional hazards models)"),
...
(`beta`) and potentially the residual standard deviation (`sigma_main`),
for example in linear or Gamma models. For models that do have a second 
parameter which is not the standard deviation, that other second parameter
is monitored (the shape parameter for parametric survival models and the
parameter `tau` in beta regression models).
