...
# select all imputed values for 'WC' (4th column of Mc)
sub3 <- grep('Mc\\[[[:digit:]]+,4\\]', parameters(mod13d), value = TRUE)
sub3
...
ri <- grep('^b_bmi\\[[[:digit:]]+,1\\]$', colnames(mod13e$MCMC[[1]]), value = T)
rs <- grep('^b_bmi\\[[[:digit:]]+,2\\]$', colnames(mod13e$MCMC[[1]]), value = T)
...
matplot(pred$newdata$age, pred$newdat[, c('bmi.fit', 'bmi.2.5%', 'bmi.97.5%')],
