...
variables. Design matrices are always named `M_<level>`.


The first eight rows of `M_ID` in the data dat is passed to JAGS (which is stored
...
variables we would have to specify a matrix that had to look like this:
```{r}
...
are columns containing the dummy variables (columns 4 and 5) as well as the
original version of `SMOKE` (first column).
The dummy variables are calculated (not sampled) in the JAGS model and are thus
completely empty in the data matrix and the matrix of initial values.

The corresponding part of the JAGS model is:
```{r}
cat(paste0('[...]\n', 
           paste0(a4mod[64:65], collapse = "\n"),
           '\n\n[...]\n',
           paste0(a4mod[74:77], collapse = "\n"),
           '\n\n[...]'))
```
