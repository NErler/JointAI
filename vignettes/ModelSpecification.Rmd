...
[default](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/family.html)
is used.
...
Interactions between multiple variables can be specified using brackets:
```{r}
...
[`sqrt()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/MathFun.html) (the square root) or [`exp()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/Log.html) (the exponential function) or by specifying a function of a variable using
[`I()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/AsIs.html),
for example a quadratic term of a variable `x`: `I(x^2)`.
...
```{r}
list_impmodels(mod3b, priors = F, regcoef = F, otherpars = F)
```
...
calculated from the imputed values. Likewise, `creat` and `albu` in `mod3c` 
...
`ilogit`. In R, this function does not exist, but the inverse logit is
available as the distribution function of the logistic distribution `plogis`.

```{r}
# Define a function ilogit
ilogit <- plogis
...
# define the inverse logit (in case you haven't done it in the previous example)
ilogit <- plogis
...
* `random = ~time + I(time^2)`: random intercept, slope and quadratic effect for `time`
...
### Specification of imputation mode types
...
# extract the chosen imputation methods
...
(decreasing), and each model has the complete covariates and all incomplete
variables that appear earlier in the sequence in its linear predictor:
```{r}
# number of missing values in the covariates in mod8a
colSums(is.na(NHANES[, names(newmeth)]))

# print information on the imputation models
# (and omit everything but the predictor variables)
...
contain information on unobserved values.
Good auxiliary variables are associated with an incomplete variable of interest
or the missingness of that variable and do not have too many missing values
themselves. Importantly, they should be observed for a large proportion of the
...
For example, we might consider the variables `educ` and `smoke` as predictors
...
```{r}
list_impmodels(mod9a, priors = FALSE, regcoef = FALSE, otherpars = FALSE,
               refcat = FALSE)
```
...
mod9b <- lm_imp(SBP ~ gender + age + occup, data = NHANES,
                ...
                trunc = list(WC = c(1e-10, 1e10)),
                n.adapt = 0, mess = FALSE)
```
...
was observed and zero otherwise. The regression coefficients of the dummy
variables are interpreted as the effect of being in the given category
...
To help to specify the reference category, the function `set_refcat()` can be
used.
```{r, echo = FALSE}
...
```{r}
set_refcat(NHANES, formula(mod10b))
```
It lists the categorical variables in the model and the options for specifying
