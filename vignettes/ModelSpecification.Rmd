...
of the incomplete variables. When the vector supplied to `models` only contains
...
The function [`get_models()`](https://nerler.github.io/JointAI/reference/get_imp_meth.html),
...
```{r}
mod8a_models <- get_models(SBP ~ age + gender + WC + alc + bili + occup + smoke,
                         data = NHANES)
mod8a_models
```
...
colSums(is.na(NHANES[, names(mod8a_models$models)]))
...
By re-ordering the elements in `models`, the order of the sequence of imputation
