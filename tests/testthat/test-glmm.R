...
if (identical(Sys.getenv("NOT_CRAN"), "true")) {
  ...
  run_glmm_models <- function() {
    ...
    invisible(force(suppressWarnings({
      models = list(
        ...
        m4b = lme_imp(
          c1 ~ c2 + b2 + p2 + L1mis + (1 | id),
          data = longDF,
          n.adapt = 5,
          n.iter = 10,
          seed = 2020,
          models = c(
            c2 = "glmm_gaussian_inverse",
        ...
        # complex structures
        m5a = lme_imp(
          y ~ M2 + o2 * abs(C1 - c2) + log(C1) + time + I(time^2) + (time | id),
          data = longDF,
          n.adapt = 5,
          n.iter = 10,
          seed = 2020,
        ...
        m5b = glme_imp(
          b1 ~ L1mis + abs(c1 - C2) + log(Be2) + time + (time + I(time^2) | id),
          data = longDF,
          n.adapt = 5,
          n.iter = 10,
          seed = 2020,
        ...
        m6b = glme_imp(
          b1 ~ c1 + C2 + B1 + time + (0 + time + I(time^2) | id),
          data = longDF,
          n.adapt = 5,
          n.iter = 10,
          seed = 2020,
  ...
  test_that("prediction works", {
    for (k in seq_along(models)) {
      expect_s3_class(
        predict(models[[k]], type = "link", warn = FALSE)$fitted,
        "data.frame"
      )
      expect_s3_class(
        predict(models[[k]], type = "response", warn = FALSE)$fitted,
        "data.frame"
      )
    }
    ...
    expect_s3_class(
      predict(models$m5a, newdata = ndf, warn = FALSE)$fitted,
      "data.frame"
    )
  })
  ...
  test_that("residuals", {
    for (k in seq_along(models)[8]) {
      expect_type(
        residuals(models[[k]], type = "response", warn = FALSE),
        "double"
      )
      ...
      if (models[[k]]$analysis_type %in% c("beta", "glmm_beta")) {
        ...
        expect_type(
          residuals(models[[k]], type = "working", warn = FALSE),
          "double"
        )
        expect_type(
          residuals(models[[k]], type = "pearson", warn = FALSE),
          "double"
        )
      }
    }
  ...
  test_that("model can be plottet", {
    for (i in seq_along(models)) {
      if (
        models[[i]]$analysis_type %in%
          c("lognorm", "glmm_lognorm", "beta", "glmm_beta")
      ) {
