...
JAGSmodel_survreg <- function(info) {
  ...
  paste_ppc <- if (info$ppc) {
    paste0(
      ...
      tab(4), "mu_", info$varname, "[", info$index, "] <- 1/rate_", y_name,
  ...
  paste0(tab(2), "# Weibull survival model for ", info$varname, "\n",
         ...
         info$index, "], ", info$resp_mat, "[", info$index, ", ", info$resp_col[1],
         "])", "\n",
...
JAGSmodel_coxph <- function(info) {
  ...
  paste0(tab(), "# Cox PH model for ", info$varname, "\n",
         ...
         Mc_predictor, "\n",
         tab(4), "logh0_", info$varname, "[", info$index, "] <- inprod(",
         info$parname, "_B_", info$varname, "[], B_", info$varname, "[",
         info$index, ", ])", "\n",
         tab(4), "logh_", info$varname, "[", info$index, "] <- logh0_",
         info$varname, "[", info$index, "] + eta_", info$varname, "[",
         info$index, "]", "\n\n",
         tab(4), "for (k in 1:15) {", "\n",
         tab(6), "logh0s_", info$varname, "[", info$index, ", k] <- inprod(",
         info$parname, "_B_", info$varname, "[], Bs_", info$varname,
         "[15 * (", info$index, " - 1) + k, ])", "\n",
         tab(6), "S_", info$varname, "[", info$index, ", k] <- gkw[k] * exp(logh0s_",
         info$varname, "[", info$index, ", k])", "\n",
         tab(4), "}", "\n\n",
         tab(4), "log.survival[", info$index, "] <- -exp(eta_", info$varname, "[",
         info$index, "]) * ", info$resp_mat ,"[", info$index, ", ",
         info$resp_col[1], "]/2 * sum(S_",  info$varname, "[", info$index, ", ])", "\n",
         tab(4), "phi_", info$varname, "[", info$index, "] <- 5000 - ((",
         info$resp_mat ,"[", info$index, ", ", info$resp_col[2], "] * logh_",
         info$varname, "[", info$index, "])) - (log.survival[", info$index, "])", "\n",
         tab(4), "zero_", info$varname, "[", info$index, "] ~ dpois(phi_", info$varname,
         "[", info$index, "])", "\n",
         tab(), "}\n\n",
         ...
         tab(), "for (k in 1:6) {", "\n",
         tab(4), info$parname, "_B_", info$varname, "[k] ~ dnorm(mu_reg_surv, tau_reg_surv)",
         "\n",
         tab(), "}"
  )
}
