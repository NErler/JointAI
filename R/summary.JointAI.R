...
summary.JointAI <- function(
  ...
  res_list <- sapply(
    ...
    function(varname) {
      rdnam <- nlapply(
        names(object$info_list[[varname]]$rd_vcov),
        function(lvl) {
          if (isTRUE(object$info_list[[varname]]$rd_vcov[[lvl]] == "full")) {
            paste0(
              "^",
              c("b", "D", "invD", "RinvD", "KinvD"),
              attr(object$info_list[[varname]]$rd_vcov[[lvl]], "name"),
              "_",
              lvl,
              "\\b"
            )
      ...
      MCMCsub <- MCMC[,
        intersect(
          ...
          c(
            ...
            grep(
              paste0("_", object$info_list[[varname]]$varname, "\\b"),
              colnames(MCMC),
              value = TRUE
            ),
            grep(
              paste0("_", object$info_list[[varname]]$varname, "_"),
              colnames(MCMC),
              value = TRUE
            ),
      ...
      if (ncol(MCMCsub) > 0) {
        ...
        colnames(MCMCsub)[na.omit(match(
          object$coef_list[[varname]]$coef,
          colnames(MCMCsub)
        ))] <-
        ...
        stats[, paste0(quantiles * 100, "%")] <- t(apply(
          MCMCsub,
          2,
          quantile,
          quantiles
        ...
        regcoef <- stats[
          intersect(rownames(stats), object$coef_list[[varname]]$varnam_print),
          ,
          drop = FALSE
        ]

        sigma <- if (
          object$info_list[[varname]]$family %in%
        ...
        intercepts <- if (
          object$info_list[[varname]]$modeltype %in%
            c("clm", "clmm")
        ) {
          get_intercepts(
            stats,
            varname,
            levels(object$Mlist$refs[[varname]]),
        ...
        rd_vcov <- if (!is.null(object$info_list[[varname]]$hc_list)) {
          Dpat <- nlapply(object$Mlist$idvar, function(lvl) {
            if (isTRUE(object$info_list[[varname]]$nranef[lvl] > 0L)) {
              if (
                isTRUE(object$info_list[[varname]]$rd_vcov[[lvl]] == "full")
              ) {
                paste0(
                  "^D[[:digit:]]*_",
                  lvl,
                ...
                paste0(
                  "^D_",
                  object$info_list[[varname]]$varname,
                  "_",
                  lvl,
                  "\\[[[:digit:]]+,[[:digit:]]+\\]"
                )
          ...
          Ds <- nlapply(names(Dpat), function(lvl) {
            D <- stats[
              grep(Dpat[[lvl]], rownames(stats), value = TRUE),
              ,
              drop = FALSE
            ]
            ...
            if (nrow(D) > 0) {
              ...
              attr(D, "warnings") <- attr(
                object$info_list[[varname]]$hc_list,
                "warnings"
              )
              ...
              for (k in which(names(object$Mlist$rd_vcov[[lvl]]) == "full")) {
                if (varname %in% object$Mlist$rd_vcov[[lvl]][[k]]) {
                  attr(D, "ranef_index") <- attr(
                    object$Mlist$rd_vcov[[lvl]][[k]],
                    "ranef_index"
                  )
                }
              }
        ...
        assoc_type <- if (object$info_list[[varname]]$modeltype %in% "JM") {
          object$info_list[[which(
            sapply(object$info_list, "[[", "modeltype") == "JM"
          )]]$assoc_type
        }

        wb_shape <- if (
          object$info_list[[varname]]$modeltype %in% c("survreg")
        ) {
          stats[
            c(paste0("shape_", object$info_list[[varname]]$varname)),
            -which(colnames(stats) == "tail-prob."),
            drop = FALSE
          ]
        }
        ...
        list(
          modeltype = object$info_list[[varname]]$modeltype,
  ...
  out$nchain <- coda::nchain(object$MCMC) -
    sum(
      exclude_chains %in%
...
#' @export
print.summary.JointAI <- function(
  x,
  ...
  if (sum(!sapply(x$res, is.null)) > 1 | !is.null(x$outcome)) {
    ...
    cat(
      "Bayesian",
      print_type(x$res[[1]]$modeltype, x$res[[1]]$family),
      "fitted with JointAI\n"
    )
  }
    ...
    if (!is.null(x$res[[k]])) {
      ...
      if (sum(!sapply(x$res, is.null)) > 1 | !is.null(x$outcome)) {
        cat(paste0(
          ...
          "  ",
          "Bayesian ",
          print_type(x$res[[k]]$modeltype, x$res[[k]]$family),
          " for ",
      ...
      if (!is.null(x$res[[k]]$rd_vcov)) {
        ...
        for (lvl in names(x$res[[k]]$rd_vcov)) {
          ...
          print_d <- isTRUE(rd_vcov == "full") &&
            (w == 1 ||
              if (w > 1) {
                all(lvapply(x$res[names(ranef_index)[seq_len(w - 1)]], is.null))
              })
          ...
          if (print_d || isTRUE(rd_vcov != "full")) {
            ...
            w_ref <- min(which(
              !lvapply(x$res[names(ranef_index)[seq_len(w - 1)]], is.null)
            ))
            cat(paste0(
              tab(),
              "(see model for ",
...
coef.JointAI <- function(
  ...
  nlapply(names(object$fixed), function(k) {
    ...
    names(cfs)[match(reg_coefs, names(cfs))] <- x$varname[match(
      reg_coefs,
      x$coef
    )]
...
coef.summary.JointAI <- function(
  ...
  Filter(Negate(is.null), lapply(object$res, "[[", "regcoef"))
}
...
confint.JointAI <- function(
  ...
  if (!is.null(subset) & !is.null(parm)) {
    errormsg(
      "At least one of %s and should be NULL.",
      dQuote("parm"),
      dQuote("subset")
    )
  }
...
print.JointAI <- function(x, digits = max(4, getOption("digits") - 4), ...) {
  ...
  if (!is.null(MCMC)) {
    ...
    for (k in seq_along(coefs)) {
      ...
      intercepts <- if (
        x$info_list[[varname]]$modeltype %in%
          c("clm", "clmm")
      ) {
        stats <- t(t(colMeans(MCMC[, grep(
          paste0("gamma_", varname),
          colnames(MCMC)
        )])))
        get_intercepts(
          stats,
      ...
      cat(
        ...
        "Bayesian",
        print_type(
          x$info_list[[varname]]$modeltype,
          x$info_list[[varname]]$family
        ),
        "for",
        dQuote(varname),
        "\n"
      ...
      if (
        x$info_list[[names(coefs)[k]]]$modeltype %in%
        ...
        if (
          ...
          print(
            c(
              setNames(c(intercepts), rownames(intercepts)),
      ...
      if (paste0("sigma_", varname) %in% colnames(MCMC)) {
        ...
        print(
          colMeans(MCMC[, paste0("sigma_", names(coefs)[k]), drop = FALSE]),
          digits = digits
        )
      }
    }
...
get_missinfo <- function(object) {
  ...
  complcases <- lapply(names(groups), function(k) {
    cc <- complete.cases(object$data[
      match(unique(groups[[k]]), groups[[k]]),
  ...
  miss_list <- sapply(
    ...
    function(lvl) {
      subdat <- object$data[
        match(unique(object$Mlist$groups[[lvl]]), object$Mlist$groups[[lvl]]),
  ...
  list("complete_cases" = do.call(rbind, complcases), miss_list = miss_list)
}
