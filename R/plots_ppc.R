plot_ppc <- function(object, fun, var, type = c('histogram', 'density', 'chainwise_dens'), ...) {
  ...
  if (type == 'chainwise_dens')
    p <- p + geom_density(aes(color = chain), ...)


  p <- p + geom_vline(data = value_obs, aes(xintercept = value))

  if (object$analysis_type == 'survreg')
    print(p + facet_wrap('group'))
  else print(p)
...
plot_resid_binned <- function(object, nbins, nit, var,
  ...
  if (type == 'quantiles') {
    ...
    if (type == 'trajectories')
      p <- ggplot(binDFrep) +
        geom_line(aes(x = mean_var_exp, y = mean_resid_rep, group = factor(iters)), alpha = 0.3, color = grey(0.5)) +
        geom_line(data = binDFobs, aes(x = x, y = y, color = iters, group = factor(iters)), lwd = 1) +
        geom_hline(yintercept = 0) +
        theme(legend.position = 'none')
    ...
    if (type == 'dots')
      p <- ggplot(binDFrep) +
        geom_point(aes(x = mean_var_exp, y = mean_resid_rep, group = factor(iters)), alpha = 0.3, color = grey(0.5)) +
