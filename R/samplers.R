...
MH_algo <- function(init, iters, logPost, proposal_d, proposal_r, pen = 1e-16, ...) {
...
logPost_b <- function(y, Xc, Xic, Xl, Xil, Z, beta, sig_y, b, D, hc_beta, type, ...) {
  eta <- Z %*% b -
    Xl %*% beta[, colnames(Xl)] -
    Xil %*% beta[, colnames(Xil)]


  part1 <- switch(type,
                  gaussian = sum(-0.5 * (y - eta)^2 / sig_y^2),
                  binomial = sum(-log(1 + exp(eta)) + y * eta),
                  poisson = sum(y * eta - exp(eta) - log(factorial(y))),
                  Gamma = sum(dgamma(DF$y, shape = eta^2/sig_y, rate = abs(eta)/sig_y, log = T))
  )


  mu_b <- Xc %*% beta[, colnames(Xc)] + Xic %*% beta[, colnames(Xic)]
  if (length(hc_beta) > 0) {
    for (kk in seq_along(hc_beta)) {
      mu_b <- c(mu_b,
...
ranef_sample <- function(object, newdata, subset = object$monitor_params,
                         start = NULL, end = NULL, thin = NULL, exclude_chains = NULL,
  ...
  if (attr(object$analysis_type, 'family') %in% c('gaussian', 'Gamma')) {
    y <- newdata[, colnames(object$Mlist$y)]
    sig <- try(MCMC[, paste0("sigma_", colnames(object$Mlist$y))])
  } else if (attr(object$analysis_type, 'family') == 'binomial') {
    y <- as.numeric(newdata[, colnames(object$Mlist$y)]) -
      min(as.numeric(newdata[, colnames(object$Mlist$y)]))
  } else if (attr(object$analysis_type, 'family') == 'poisson') {
    y <- newdata[, colnames(object$Mlist$y)]
  } else {
    stop('unknown family')
  }
  ...
  #     chol2inv(chol(Z %*% Darr[, , k] %*% t(Z) + sig[k]^2 * diag(nrow(Z)))),
  ...
  #       chol2inv(chol(t(X) %*% invVarr[, , k] %*% X * object$Mlist$N + pen * diag(ncol(X)))) %*%
  #       t(X) %*% invVarr[, , k],
  #     FUN.VALUE = matrix(nrow = nrow(Z), ncol = nrow(Z), data = 0)
  #   )
  #
  ...
  hc_beta <- lapply(object$Mlist$hc_list,
                    function(k) {
  ...
  start_points <- optim(par = rep(0, ncol(Z)),
                        ...
                        beta = t(as.matrix(colMeans(beta))), hc_beta = hc_beta,
                        sig_y = mean(sig), D = apply(Darr, 1:2, mean), Z = Z,
                        type = attr(object$analysis_type, "family"),
  ...
  for (k in 1:nrow(MCMC)) {
    ...
    #                proposal_r = function(...) mvtnorm::rmvnorm(n = 1, sigma = vars[, , k], ...))
                   proposal_d = function(...) mvtnorm::dmvnorm(sigma = adj * chol2inv(chol(-start_points$hessian)), ...),
                   proposal_r = function(...) mvtnorm::rmvnorm(n = 1, sigma = adj * chol2inv(chol(-start_points$hessian)), ...))
